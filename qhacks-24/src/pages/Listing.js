...
export default function Listing() {
    ...
    const handleClick = (condition, description, postalcode, price, title, category) => {
        // Update exampleData before navigating to Item.js
        const updatedData = [
            {
              condition: condition,
        ...
        return (
            <div className="flex justify-between m-[20px]">
                <Link
                    to='/item'
                    className="w-full"
                    onClick={() => handleClick(data.title, data.description, data.postalcode, data.price, data.title, data.category)}
                >
                    <div 
